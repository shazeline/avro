...
class TestSchema(unittest.TestCase):
  ...
  def test_enum_valid_type(self):
    enum_schema = ExampleSchema("""{
      "name": "foo",
      "fields": [{"symbols": ["A", "B"], "type": "enum", "name": "col"}],
      "type": "record"}""", True)

    record_schema = schema.parse(enum_schema.schema_string)
    enum_field = record_schema.fields[0]
    self.assertIsInstance(enum_field.type, schema.EnumSchema)
    self.assertEqual(['A','B'], enum_field.get_prop('symbols'))
